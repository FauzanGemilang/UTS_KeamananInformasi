...
MAX_UPLOAD_MB = int(
    os.environ.get("MAX_UPLOAD_MB", "3")
)
        ...
        with Image.open(
            ...
            if width * height > MAX_IMAGE_PIXELS:
                raise ValueError(
                    ...
                    "Silakan gunakan gambar "
                    "dengan resolusi lebih kecil."
                )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not original_bytes:
                raise ValueError(
                    "File gambar yang diunggah kosong."
                )
            ...
            # Hitung kapasitas LSB
            capacity = capacity_bytes(
                original_bytes
            )

            # Periksa kapasitas gambar
            ...
            # Nama file hasil
            filename = (
                f"{get_original_name(uploaded_file.filename)}"
                "_stego.png"
            )
...
def request_entity_too_large(error):
    ...
    flash(
        f"Ukuran unggahan melebihi batas "
        f"{MAX_UPLOAD_MB} MB.",
        "error"
    )
